...
pub fn send(socket: &UdpSocket, fighter: &characters::characterAbstract::Fighter, player_number: u8){
	let pstate = CharacterState::new(fighter.char_state.particle.clone(), fighter.char_state.state, fighter.char_state.frames_per_state(), fighter.char_state.current_frame(), fighter.char_state.frame_count, fighter.char_state.direction);
